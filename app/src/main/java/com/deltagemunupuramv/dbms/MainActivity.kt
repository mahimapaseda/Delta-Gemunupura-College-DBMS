...
class MainActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            ...
            R.id.nav_init_sample_data -> {
                val currentUser = UserSession.getUser()
                if (currentUser != null && AccessLevel.canInitializeSampleData(currentUser.role)) {
                    initializeSampleData()
                } else {
                    Toast.makeText(this, "You don't have permission to initialize sample data", Toast.LENGTH_SHORT).show()
                }
            }
            R.id.nav_profile -> {
    ...
    private fun syncFirebaseData() {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            ...
            if (success) {
                Toast.makeText(this@MainActivity, "Data synchronized successfully!", Toast.LENGTH_LONG).show()
            } else {
                Toast.makeText(this@MainActivity, "Failed to synchronize data. Check your internet connection.", Toast.LENGTH_LONG).show()
            }
        }
    }
    ...
    private fun initializeSampleData() {
        ...
        CoroutineScope(Dispatchers.Main).launch {
            ...
            if (success) {
                Toast.makeText(this@MainActivity, "Sample data initialized successfully!", Toast.LENGTH_LONG).show()
            } else {
                Toast.makeText(this@MainActivity, "Failed to initialize sample data.", Toast.LENGTH_LONG).show()
            }
        }
    }
